  ...
  case HTTP_EVENT_ON_DATA:
    ...
    if (!esp_http_client_is_chunked_response(evt->client)) {
      ...
      if (evt->user_data) {
        ...
        // Fallback to static output_buffer if user_data is not provided
        if (output_buffer == NULL) {
          // We initialize output_buffer with 0 because it is used by strlen()
          // and similar functions therefore should be null terminated.
          // For chunked encoding, content_len might be -1, so use MAX_HTTP_OUTPUT_BUFFER as limit
          size_t alloc_size = MAX_HTTP_OUTPUT_BUFFER + 1; 
          output_buffer = (char *)calloc(alloc_size, sizeof(char));
          output_len = 0;
          if (output_buffer == NULL) {
